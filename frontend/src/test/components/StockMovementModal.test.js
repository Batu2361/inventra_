...
describe('StockMovementModal', () => {
    it('renders with the product name', async () => {
        renderWithToast(_jsx(StockMovementModal, { ...defaultProps }));
    ...
    it('submits INBOUND movement with correct payload', async () => {
        ...
        await waitFor(() => {
            expect(mockedApi.post).toHaveBeenCalledWith('/products/prod-uuid-123/movements', expect.objectContaining({ type: 'INBOUND', quantity: 25 }));
        });
    });
    ...
    it('close button calls onClose', async () => {
        const onClose = vi.fn();
        renderWithToast(_jsx(StockMovementModal, { ...defaultProps, onClose: onClose }));
